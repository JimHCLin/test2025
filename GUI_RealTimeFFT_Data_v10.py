...
class RealtimePlotApp(ctk.CTk):
    ...
    def compute_and_plot_fft(self):
        ...
        # 計算最大振幅與基頻差異
        valid_indices = [i for i, f in enumerate(self.top_frequencies) if f > 30]
        analysis_result = ""
        ...
        if valid_indices:
            ...
            analysis_result += f"\n基頻: {base_freq:.2f} Hz，振幅: {base_amp:.2f}\n"
            analysis_result += f"最大振幅: {max_amp:.2f}（頻率: {max_freq:.2f} Hz）\n"
            analysis_result += f"最大振幅與基頻差異: {percent_diff:.2f}%\n"
        else:
            analysis_result += "\n找不到大於 30 Hz 的基頻\n"
        # 顯示到文字框
        self.csv_textbox.delete(1.0, ctk.END)
        self.csv_textbox.insert(ctk.END, analysis_result)
        self.canvas.draw()
    ...
    def save_chart_as_pngerrrr(self):
        save_path = filedialog.asksaveasfilename(defaultextension=".png", filetypes=[("PNG files", "*.png")])
        if save_path:
            self.fig.savefig(save_path)
            print(f"圖表已儲存至: {save_path}")

    def export_data_to_csvss(self):
        save_path = filedialog.asksaveasfilename(defaultextension='.csv', filetypes=[("CSV files", "*.csv")])
        if save_path:
            self.df.to_csv(save_path, index=False)
            self.fft_top_df.to_csv(save_path, index=False)
            messagebox.showinfo("匯出成功", f"資料已匯出至:\n{save_path}")
