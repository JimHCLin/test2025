...
class RealtimePlotApp(ctk.CTk):
    ...
    def load_csvdddddddds(self):
        ...
        if file_path:
            ...
            required_cols = ['Xaxis', 'Yaxis', 'Zaxis']
            if not all(col in self.df.columns for col in required_cols):
                print("CSV 檔案缺少 ax、ay 或 az 欄位")
                return

            # 將欄位轉為數值型態
            self.df['ax'] = pd.to_numeric(self.df['ax'], errors='coerce')
            self.df['ay'] = pd.to_numeric(self.df['ay'], errors='coerce')
            self.df['az'] = pd.to_numeric(self.df['az'], errors='coerce')
            self.df = self.df.dropna(subset=['ax', 'ay', 'az'])

            # 計算 RMS = sqrt(ax² + ay² + az²)
            rms = np.sqrt(self.df['ax']**2 + self.df['ay']**2 + self.df['az']**2)
            self.y_data = rms.tolist()
    ...
    def print_all_data(self):
        ...
        #self.df['ax']
        print("ax:")
        print(self.df['ax'].head(20).to_string(index=False))
        #print(f"ax數值:{self.df['ax'].head(20)}\n")
        #print(f"ay數值:{self.df['ay'].head(20)}\n")
        #print(f"az數值:{self.df['az'].head(20)}\n")
