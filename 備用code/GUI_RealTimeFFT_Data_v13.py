...
class RealtimePlotApp(ctk.CTk):
    ...
    def batch_process_folder_new_thread(self):
        thread = threading.Thread(target=self.batch_process_folder)
        thread.start()
    ...
    def batch_process_folder(self):
        ...
        #self.progress.configure(maximum=total_files_length)
        ####
        ...
        # 假設 tsv_files 是一個包含所有檔案名稱的列表
        for idx, tsv_file in enumerate(tsv_files, 1):  # 使用 enumerate 加入計數器，從 1 開始
            file_path = os.path.join(folder_path, tsv_file)
            self.filename_label.configure(text=f"處理檔案: {tsv_file} ({idx}/{len(tsv_files)})")  # 顯示目前檔案的處理進度
            ...
            try:
                ...
                # 更新進度條
                self.progress.set(idx + 1)  # 更新進度條為當前處理的步驟
                # 呼叫下一次處理，並且插入短暫延遲
                self.after(100, self.batch_process_folder, tsv_files, idx + 1)
                ###
            except Exception as e:
    ...
    def update_progress(self, value):
        # 更新進度條，這裡是使用 `after` 方法來確保更新操作是在主線程中執行
